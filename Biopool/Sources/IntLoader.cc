...
void IntLoader::loadGroup(Group& group)
{
  ...
  for (unsigned long i= 0; i < nAtoms; i++) 
      ...
      if( i > 2 )
	{
	  ...
	  tmpV = group[i-1].getCoords();
	  if(tmpV[0] == 0.0)
	    { // Special case : as long as atoms remain linear with the 
...
void IntLoader::loadSpacer(Spacer& sp)
{
  ...
  if(specialCase_i3_bLP1) 
    sp.getAmino(0)[2].setCoords(vSave_i3_bLP1_forSpecialCase[0], 
       vSave_i3_bLP1_forSpecialCase[1], bondLength_forSpecialCase
	   * cos(bondAngle_forSpecialCase / 57.29577951308232088));
