...
public:
  vgVector3() {};
  vgVector3(T x, T y, T z = (T)0) { this->x = x; this->y = y; this->z = z; };
  vgVector3(const vgVector3<T>& other) { x = other.x; y = other.y; z = other.z; };
  vgVector3<T>& operator= (const vgVector3<T>& other) { x = other.x; y = other.y; z = other.z; return *this; };
  T sum() const { return x+y+z; };
  T square() const { return x*x+y*y+z*z; };
  vg_ieee64 length() const { return sqrt((vg_ieee64)square()); };
