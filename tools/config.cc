...
config::existParameter(string paramName)
{
  ...
  while(i<param.size())
    {
      if(param[i++].name.compare(paramName)==0) { return 1; }
    } 
  return 0; 
}
...
template<class Tget> Tget config::getParameter(string paramName,Tget tmp)
{
  ...
  if(i==param.size()) { ERROR("parameter not found",exception); }  // Entry not found
...
template <class Tset> int config::setParameter(string paramName,Tset value)
{
  ...
  while( (i<param.size()) && !found)
    {
      if(param[i++].name.compare(paramName)==0) { found=1; }
    }
  if(!found) { return 0; }  // Entry not found
...
template<class Tchange> int config::changeParameter(string paramName,Tchange value)
{
  ...
  string aktline;
  int finish=0;
  if(!setParameter(paramName,value)) { return 0; }  // first update the parameter at the heap
  while((i<param.size()) && (param[i].name.compare(paramName)!=0))
    {
      i++;
    }
  if(i==param.size()) { return 0; }  // Entry not found 
...
template<class Tnew> int config::newParameter(string fileName,string paramName,Tnew value)
{
  string tmpline;
  parameter tmpparam;
  if(existParameter(paramName)) { return 0; }
  tmpparam.name=paramName;
  tmpparam.file=fileName;
  param.push_back(tmpparam);
  if(!existParameter(paramName) || (!setParameter(paramName,value))) { return 0; } // well there is something wrong !
       ...
       else
	 {
	   ...
	   if((pos!=anfpos) && !tmpstr.empty())   // If no comment and not empty
	       ...
	       else
		 {
	       ...
	       if(anf==string::npos) { ERROR("Parameter without value.",exception); } 
