	...
	double& A, double& B)
{
	...
	for (iter=0;iter<max_iter;iter++)
	{
		...
		{
			info((char *)"Line search fails in two-class probability estimates\n");
			break;
		}
	}

	if (iter>=max_iter)
		info((char*)"Reaching maximal iterations in two-class probability estimates\n");
	free(t);
}
...
void multiclass_probability(int k, double **r, double *p)
{
	...
	if (iter>=max_iter)
		info((char*)"Exceeds max_iter in multiclass_prob\n");
	...
	const svm_problem *prob, const svm_parameter *param)
{
	...
	for(i=0;i<prob->l;i++)
	        if (fabs(ymv[i]) > 5*std) 
		        ...
		        mae+=fabs(ymv[i]);
	mae /= (prob->l-count);
	info((char*)"Prob. model for test data: target value = predicted value + z,\nz: Laplace distribution e^(-|z|/sigma)/(2sigma),sigma= %g\n",mae);
	free(ymv);
	return mae;
}
...
double svm_get_svr_probability(const svm_model *model)
{
	...
	{
		info((char *)"Model doesn't contain information for SVR probability inference\n");
		return 0;
	}
}
	...
	while(1)
	{
		...
		{		
			...
			if(kernel_type_table[i] == NULL)
			{
			 ...
			 ifc=fscanf(fp,"%lf",&param.degree);
		else if(strcmp(cmd,"gamma")==0)
			 ifc=fscanf(fp,"%lf",&param.gamma);
		else if(strcmp(cmd,"coef0")==0)
			 ifc=fscanf(fp,"%lf",&param.coef0);
		else if(strcmp(cmd,"nr_class")==0)
			 ifc=fscanf(fp,"%d",&model->nr_class);
		else if(strcmp(cmd,"total_sv")==0)
			 ifc=fscanf(fp,"%d",&model->l);
		else if(strcmp(cmd,"rho")==0)
		{
			int n = model->nr_class * (model->nr_class-1)/2;
			model->rho = Malloc(double,n);
			for(int i=0;i<n;i++)
				 ifc=fscanf(fp,"%lf",&model->rho[i]);
		}
		...
		else if(strcmp(cmd,"probA")==0)
		{
			...
			for(int i=0;i<n;i++)
				 ifc=fscanf(fp,"%lf",&model->probA[i]);
		}
		else if(strcmp(cmd,"probB")==0)
		{
			...
			for(int i=0;i<n;i++)
				 ifc=fscanf(fp,"%lf",&model->probB[i]);
		}
		...
		{
			fprintf(stderr,"unknown text in model file %d \n",ifc);
			free(model->rho);
			free(model->label);
			free(model->nSV);
			free(model);
...
out:
	...
	for(i=0;i<l;i++)
	{
		...
		for(int k=0;k<m;k++)
			ifc=fscanf(fp,"%lf",&model->sv_coef[k][i]);
		while(1)
		{
			int c;
			do {
