/**
*@Class:              PhiPsiOmega
*@Author:             Alessandro Albiero
*@Modified by:        Andrea Bazzoli
*@Project Name:       Victor
...
long double 
PhiPsiOmega::calculateEnergy(Spacer& sp)    {
  long double en = 0.0;
...
long double 
PhiPsiOmega::calculateEnergy(Spacer& sp, unsigned int index1, 
				      unsigned int index2)    {
  long double en = 0.0;
...
long double 
PhiPsiOmega::calculateEnergy(AminoAcid& aa)    {
...
*/
long double 
PhiPsiOmega::calculateEnergy(AminoAcid& diheds, AminoAcidCode code) {
...
inline int
PhiPsiOmega::sGetPropOmegaBin(double p)    {
  int x = -1;
  if (RANGE_OMEGA == 11)    {
      if ((p < -20) && (p >= -150) )
	x = 0;
...
inline void 
PhiPsiOmega::sAddProp(int code, int x, int y, int z)    {
...
inline int 
PhiPsiOmega::sGetPropBin(double p)    {
...
inline int 
PhiPsiOmega::setRange_Omega(int n)    {
   if (n == 1)
   { 
     RANGE_OMEGA = 11;
   }
...
double
PhiPsiOmega::pGetMaxPropensities( int amino )    {
  double  max = 0.00;
...
double
PhiPsiOmega::pReturnMaxPropensities(int amino)    {
  return amino_max_propensities[amino];
}

void
PhiPsiOmega::pConstructMaxPropensities()    {
  for ( int i = 0; i < AminoAcid_CODE_SIZE; i++ )
...
*/
double
PhiPsiOmega::pGetMinPropensities(int amino)    {
...
/**
* @param   amino: the code of an amino acid type.
* @Return value:   minimum (phi, psi, omega) propensity of the amino acid type.
*/
double
PhiPsiOmega::pReturnMinPropensities(int amino)    {
  return amino_min_propensities[amino];
}


/**
* @Description    computes and stores the (phi, psi, omega) propensity for each amino acid type.
*/
void
PhiPsiOmega::pConstructMinPropensities()    {
  for(int i=0; i<AminoAcid_CODE_SIZE; i++)
...
double 
PhiPsiOmega::getOmegaAngle( int prop, long RANGE_OMEGA )    {
	if ( RANGE_OMEGA!=3 )
...
inline
double 
PhiPsiOmega::getPhiPsiAngle( int prop, long SIZE_TABLE, int ARC_STEP ) 
{
	return ARC_STEP*prop-180;
}


vector< vector<Potential::ANGLES> >* 
PhiPsiOmega::getOrderedEnergyTable()    {
	vector< vector<ANGLES> > *arr=new vector< vector<ANGLES> >;
	
	for ( int aa=0; aa<AminoAcid_CODE_SIZE; ++aa )    {
//		cout <<aminoAcidFullTranslator((AminoAcidCode)aa) <<":" <<endl;
		// I get the propensities for each amino acid:
